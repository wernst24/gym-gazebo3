import numpy as np
import rclpy
from rclpy.node import Node
from geometry_msgs.msg import Twist
from sensor_msgs.msg import LaserScan
from nav_msgs.msg import Odometry
from gymnasium import spaces
from gym_gazebo3.envs.gazebo_env import GazeboEnv


class GazeboPioneer3ATEnv(GazeboEnv):
    """
    Pioneer 3AT navigation environment using ROS2 Humble and Gazebo Classic 11
    Based on the uq-in-slam implementation with 180° LIDAR
    """
    
    def __init__(self, world_file="pioneer3at_world.world", headless=True):
        launch_file = "pioneer3at_gazebo_launch.py"
        super(GazeboPioneer3ATEnv, self).__init__(launch_file, world_file, headless)
        
        # Action space: [linear_x, angular_z]
        self.action_space = spaces.Box(
            low=np.array([-1.0, -2.0]),
            high=np.array([1.0, 2.0]),
            dtype=np.float32
        )
        
        # Observation space: 61 LIDAR readings (180° scan)
        self.observation_space = spaces.Box(
            low=0.0,
            high=30.0,
            shape=(61,),
            dtype=np.float32
        )
        
        # Initialize ROS2 node
        self.node = Node('pioneer3at_env')
        
        # Publishers and subscribers
        self.cmd_vel_pub = self.node.create_publisher(
            Twist, '/cmd_vel', 10
        )
        
        self.scan_sub = self.node.create_subscription(
            LaserScan, '/scan', self._scan_callback, 10
        )
        
        self.odom_sub = self.node.create_subscription(
            Odometry, '/odom', self._odom_callback, 10
        )
        
        # State variables
        self.scan_data = None
        self.odom_data = None
        self.goal_position = np.array([10.0, 10.0])
        self.max_episode_steps = 1000
        self.current_step = 0
        
    def _scan_callback(self, msg):
        """Callback for LIDAR scan data (180° scan with 61 readings)"""
        ranges = np.array(msg.ranges)
        # Replace inf and nan values with max range
        ranges = np.where(np.isfinite(ranges), ranges, msg.range_max)
        
        # Downsample to 61 readings if necessary
        if len(ranges) > 61:
            indices = np.linspace(0, len(ranges)-1, 61, dtype=int)
            self.scan_data = ranges[indices]
        else:
            self.scan_data = ranges
            
    def _odom_callback(self, msg):
        """Callback for odometry data"""
        self.odom_data = msg
        
    def _get_observation(self):
        """Get current LIDAR observation"""
        if self.scan_data is not None:
            return self.scan_data.astype(np.float32)
        else:
            return np.full(61, 30.0, dtype=np.float32)
            
    def _compute_reward(self):
        """Compute reward based on distance to goal and obstacle avoidance"""
        reward = 0.0
        
        if self.odom_data is None or self.scan_data is None:
            return reward
            
        # Get current position
        current_pos = np.array([
            self.odom_data.pose.pose.position.x,
            self.odom_data.pose.pose.position.y
        ])
        
        # Distance to goal reward
        distance_to_goal = np.linalg.norm(current_pos - self.goal_position)
        reward += -distance_to_goal * 0.01
        
        # Collision penalty
        min_distance = np.min(self.scan_data)
        if min_distance < 0.5:
            reward -= 100.0
            
        # Movement reward (encourage exploration)
        if hasattr(self, 'prev_position'):
            movement = np.linalg.norm(current_pos - self.prev_position)
            reward += movement * 0.1
            
        self.prev_position = current_pos.copy()
        
        # Goal reached reward
        if distance_to_goal < 1.0:
            reward += 200.0
            
        # Step penalty to encourage efficiency
        reward -= 0.1
            
        return reward
        
    def _is_collision(self):
        """Check if robot has collided with obstacle"""
        if self.scan_data is None:
            return False
        return np.min(self.scan_data) < 0.5
        
    def _is_goal_reached(self):
        """Check if robot has reached the goal"""
        if self.odom_data is None:
            return False
            
        current_pos = np.array([
            self.odom_data.pose.pose.position.x,
            self.odom_data.pose.pose.position.y
        ])
        
        distance_to_goal = np.linalg.norm(current_pos - self.goal_position)
        return distance_to_goal < 1.0
        
    def step(self, action):
        """Execute action and return observation, reward, done, info"""
        # Publish velocity command
        cmd_msg = Twist()
        cmd_msg.linear.x = float(action[0])
        cmd_msg.angular.z = float(action[1])
        self.cmd_vel_pub.publish(cmd_msg)
        
        # Spin ROS2 node to process callbacks
        rclpy.spin_once(self.node, timeout_sec=0.1)
        
        # Increment step counter
        self.current_step += 1
        
        # Get observation
        obs = self._get_observation()
        
        # Compute reward
        reward = self._compute_reward()
        
        # Check if episode is done
        terminated = self._is_goal_reached() or self._is_collision()
        truncated = self.current_step >= self.max_episode_steps
        
        info = {
            'collision': self._is_collision(),
            'goal_reached': self._is_goal_reached(),
            'distance_to_goal': np.linalg.norm(
                np.array([
                    self.odom_data.pose.pose.position.x,
                    self.odom_data.pose.pose.position.y
                ]) - self.goal_position
            ) if self.odom_data is not None else float('inf')
        }
        
        return obs, reward, terminated, truncated, info
        
    def reset(self, seed=None, options=None):
        """Reset the environment"""
        obs, info = super().reset(seed, options)
        
        # Reset step counter
        self.current_step = 0
        
        # Reset state variables
        self.scan_data = None
        self.odom_data = None
        self.prev_position = None
        
        # Randomize goal position if specified
        if options and 'randomize_goal' in options:
            self.goal_position = np.random.uniform(-10, 10, size=2)
            
        # Wait for initial data
        import time
        timeout = time.time() + 10
        while (self.scan_data is None or self.odom_data is None) and time.time() < timeout:
            rclpy.spin_once(self.node, timeout_sec=0.1)
            
        return self._get_observation(), info